...
public class Patch_ArticulatingHardpointUpdate
{
    ...
    public static void Prefix(ArticulatingHardpoint __instance)
    {
        if (!__instance.remoteOnly && __instance.autoMode)
        {
            for (int i = 0; i < __instance.hardpoints.Length; i++)
            {
                ...
                if (equip && equip is SalvoGun gun)
                {
                    if (gun.isVintage == false)
                    {
                        UpdateSalvoGun(hardpoint, gun, __instance);
                    }
                }
            }
        }
    ...
    private static void UpdateSalvoGun(ArticulatingHardpoint.Hardpoint hardpoint, SalvoGun equip, ArticulatingHardpoint articulatingHardpoint)
    {
        float t;
